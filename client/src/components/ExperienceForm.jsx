...
const ExperienceForm = ({ data, onChange }) => {
    ...
    const [genetatingIndex, setGeneratingIndex] = useState(-1);
    ...
    return (
        <div className='space-y-6'>
            <div className='flex items-center justify-between'>
                <div>
                    <h3 className='flex items-center gap-2 font-semibold text-lg text-gray-900'>Professional Experience</h3>
                    ...
                    {data.map((experience, index) => (
                        <div key={index} className='p-4 border border-gray-200 rounded-lg space-y-3'>
                            ...
                            <label className='flex items-center gap-2'>
                                ...
                                <span className='text-sm text-gray-700'>Currentlu working here</span>
                            </label>

                            <div className='space-y-2'>
                                <div className='flex items-center justify-between'>
                                    <label className='text-sm font-medium text-gray-700'>Job Description</label>
                                    <button onClick={() => generateDescription(index)} disabled={genetatingIndex === index || !experience.position || !experience.company} className='flex items-center gap-2 px-3 py-1 text-sm bg-purple-100 text-purple-700 rounded hover:bg-purple-200 transition-colors disabled:opacity-50'>
                                        {genetatingIndex === index ? <Loader2 className='size-4 animate-spin' /> : <Sparkles className='size-4' />}
                                        {genetatingIndex === index ? 'Enhancing...' : 'Enhance with AI'}
                                    </button>
                                </div>
